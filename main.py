...
# from pydotool import KEY_F17, KEY_F18, key_combination, init
import pydotool
...
def run(model: str, num_faces: int,
  ...
  """
    ...
    base_options = python.BaseOptions(model_asset_path=model, delegate=mp.tasks.BaseOptions.Delegate.GPU)
    ...
    while cap.isOpened():
        ...
        if DETECTION_RESULT:
          ...
          if face_blendshapes:
              ...
              # print(face_blendshapes[0])
              # print(type(face_blendshapes[0]))
              smileLeft_value = face_blendshapes[0][44].score
              smileRight_value = face_blendshapes[0][45].score
              print(f"{smileLeft_value:.2f} {smileRight_value:.2f}")
              if(smileLeft_value >= 0.7):
                tokey1 = True
              else:
                tokey1 = False
              if(smileRight_value >= 0.7):
                tokey2 = True
              else:
                tokey2 = False
              if tokey1 and tokey2:
                print("crouch")
                pydotool.key_seq([(pydotool.KEY_F18, pydotool.DOWN), (pydotool.KEY_F17, pydotool.UP)])
              elif tokey1 or tokey2:
                print("walk")
                pydotool.key_seq([(pydotool.KEY_F18, pydotool.UP), (pydotool.KEY_F17, pydotool.DOWN)])
              else:
                print("run")
                pydotool.key_seq([(pydotool.KEY_F18, pydotool.UP), (pydotool.KEY_F17, pydotool.UP)])
